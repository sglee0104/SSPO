"""
Preprocess DSP Business & 17k Business Book dataset
...
# Use the entire loaded data without further sampling
kept_ultrafeedback = raw_ultrafeedback_data

kept_business_book = keep_partial_data(business_book_data[0], keep_ratio=business_book_keep_ratio)

# Create new dataset by combining ultrafeedback and ultrachat
def create_combined_dataset(ultrafeedback, business_book):
    """
    ...
    for uf_example in tqdm(ultrafeedback, desc="Adding DSP business data"):
        combined_example = {
            "prompt": uf_example["instruction"],
    ...
    # Add ultrachat data
    for uc_example in tqdm(business_book, desc="Adding 17k business book data"):
        
        combined_example = {
            "prompt": uc_example["question"],
            "chosen": "",  # Empty string for chosen
            "rejected": "",  # Empty string for rejected
            "unlabeled": uc_example["answer"]
        }
        combined_data.append(combined_example)
...
combined_dataset = create_combined_dataset(kept_ultrafeedback, kept_business_book) 

# Convert combined dataset to JSON format
conversation_format = convert_to_json_format(combined_dataset)
dataset_name = f"business_fb{ultrafeedback_keep_ratio}_ch{business_book_keep_ratio}"
